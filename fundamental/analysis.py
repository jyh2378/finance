...
class Database:
    def __init__(self, load_dir: str | None = None):
        ...
        self.base_info = pd.read_parquet(f"{load_dir}/info.parquet")
        self.base_price = pd.read_parquet(f"{load_dir}/price.parquet")
        self.base_income_statement = pd.read_parquet(f"{load_dir}/income_statement.parquet")
        self.base_income_statement_quarter = pd.read_parquet(f"{load_dir}/income_statement_quarter.parquet")
        self.base_balance_sheet = pd.read_parquet(f"{load_dir}/balance_sheet.parquet")
        self.base_balance_sheet_quarter = pd.read_parquet(f"{load_dir}/balance_sheet_quarter.parquet")
        self.base_cash_flow = pd.read_parquet(f"{load_dir}/cash_flow.parquet")
        self.base_cash_flow_quarter = pd.read_parquet(f"{load_dir}/cash_flow_quarter.parquet")
        self.base_estimates = pd.read_parquet(f"{load_dir}/estimates.parquet").sort_index()

        self.sector_list = self.base_info["Sector"].replace("", None).dropna().unique().tolist()
        self.industry_list = self.base_info["Industry"].replace("", None).dropna().unique().tolist()
    ...
    def make_per_valuation(self, period: Literal[0, 1] = 1):
        ...
        agg_cols = []
        # append forward EPS
        forward_eps = self.get_forward_eps(period=period)
        ...
        self.summary_df["Upside Potential"] = upside_potential
        
    def get_prices(self):
        col = self.base_price.columns.max()
        return self.base_price[col]
    
    def get_basic_eps(self, before: int = None):
        # before: 1 = last year, 2 = 2 year ago ...
        income_statement = self.base_income_statement.sort_index(ascending=[True, False])
    ...
    def get_forward_eps(self, period: int = None):
        # period: 0 = this year, 1 = next year
        forward_eps = self.base_estimates["EPS Estimate"]
        if isinstance(period, int):
            forward_eps = forward_eps.groupby("Ticker").nth(period).droplevel(1)
        return forward_eps
    ...
    def get_tickers_from_sector(self, sector: str | None = None):
        if sector is None:
            return self.base_info.index.tolist()
        else:
            if sector not in self.base_info["Sector"].unique():
                raise ValueError(f"Sector '{sector}' not found in the database.")
            return self.base_info[self.base_info["sector"] == sector].index.tolist()
        
    def get_tickers_from_industry(self, industry: str | None = None):
        if industry is None:
            return self.base_info.index.tolist()
        else:
            if industry not in self.base_info["industry"].unique():
                raise ValueError(f"Industry '{industry}' not found in the database.")
            return self.base_info[self.base_info["industry"] == industry].index.tolist()

    def get_same_sector_tickers(self, ticker: str):
        info = self.base_info.loc[ticker]
    ...
    def get_base_pegr_df(self, period: int | None = None, growth_agg: str = "mean"):
        pegr_series = self.get_forward_pegr(period, growth_agg).replace([np.inf, -np.inf], np.nan)
    ...
    def _set_ticker(self, ticker: str | None = None):
        if ticker is None:
            ...
            if self._ticker == ticker:
                ...
                self._info = self._db.base_info.loc[ticker]
                self._price = self._db.base_price.loc[ticker].sort_index()  # sort by date
                self._income_statement = self._db.base_income_statement.loc[ticker]
                self._income_statement_quarter = self._db.base_income_statement_quarter.loc[ticker]
                self._balance_sheet = self._db.base_balance_sheet.loc[ticker]
                self._balance_sheet_quarter = self._db.base_balance_sheet_quarter.loc[ticker]
                self._cash_flow = self._db.base_cash_flow.loc[ticker]
                self._cash_flow_quarter = self._db.base_cash_flow_quarter.loc[ticker]
                self._estimates = self._db.base_estimates.loc[ticker]
