...
def _get_duplicate_usa_tickers(usa_df):
    ...
    criteria = usa_df["Security Name"].apply(_remove_parentheses)
    for row in usa_df.itertuples():
        base = row[0]
        if base in duplicate_tickers:
            continue
        security_name = _remove_parentheses(row[1])
        target_name = _to_raw_char(_get_start_words(security_name))
